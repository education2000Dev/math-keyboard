...
class CommandButton extends React.Component {
    ...
    onClick = (value) => {
        switch (value) {
            ...
            case 'Backspace':
                ...
                break;
            case 'Clear':
                this.props.dispatch(actions.setValue('', 0));
                this.props.inputfield.val('');
                break;
            case 'Hide':
    ...
    render() {
        return (
            <div className={this.state.ClassName} onTouchStart={() => {this.onTouchStart()}} onTouchEnd={(e) => {this.onTouchEnd(e)}}>
                <SVGInline svg={this.props.src} className='command' />
            </div>
        )
    }
}
